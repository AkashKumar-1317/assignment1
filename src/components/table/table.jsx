...
export default function Table() {
  ...
  const filterPropertiesOfAlcohol = (property, value) => {
    return data.filter((data) => data[property] === value);
  };
  const mean = (Obs) => {
    return Number(Obs.reduce((prev, curr) => prev + curr) / Obs.length).toFixed(
      3
    );
  };
  const medium = (Obs) => {
    return Obs.length % 2 === 0
      ? ((Obs[Obs.length / 2] + Obs[Obs.length + 1]) * 0.5).toFixed(3)
        ...
        <tbody>
          ...
          <tr>
            ...
            {arr.map((arr) => (
              <td key={arr}>
                {mean(
                  filterPropertiesOfAlcohol("Alcohol", arr).map(
          ...
          <tr>
            ...
            {arr1.map((arr1) => (
              <td key={arr1}>
                {mean(
                  gamma(
                    filterPropertiesOfAlcohol("Alcohol", arr1).map(
                      (data) => data
                    )
                  )
                )}
